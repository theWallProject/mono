...
import { type LinkField, type valuesOfListOfReasons } from "@theWallProject/common"
      ...
      void (async () => {
        ...
        if (extraUrls.length > 0) {
          ...
          for (const url of extraUrls) {
            ...
            // Skip URLs from regex domains that didn't match (e.g., linkedin.com/legal/privacy-policy)
            if ((category === null || category === "il") && isFromRegexDomain(url)) {
              log(`  [DEBUG] SKIP (regex domain, no match): ${url}`)
              continue
            }
