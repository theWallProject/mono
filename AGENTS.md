...
3. **Screenshots**: Not generated by default. Use `--screenshots` flag or run `pnpm test:screenshots` separately (JVM-based, no emulator needed)
4. **Release**: Run `pnpm release:patch` (or minor/major) to bump version and build signed APK. Add `--screenshots` to also generate screenshots.
